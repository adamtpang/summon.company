...
summon.company is an AI-agent company platform built as an additive product layer on the
...
- Product UI: additive summon.company routes and components in `ui/`.
...
Production is deployed from `apps/landing/` to the existing Vercel project `summon.company`.
...
1. Prefer new files and routes for summon.company product behavior.
