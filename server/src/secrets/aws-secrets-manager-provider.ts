...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Summon server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Summon company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Summon-managed namespace secrets cannot be imported as external references",
  );
}
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  if (skippedForeignPaperclipSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignPaperclipSampleCount} Summon-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.paperclipManaged && sample.paperclipCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Summon-managed secrets for this company; do not import them as external references.");
      }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Summon metadata separately from AWS-managed secrets.",
          "Restoring access requires the Summon database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Summon secret ${input.context.secretName}` : undefined,
