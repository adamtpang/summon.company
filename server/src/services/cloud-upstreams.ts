  ...
  return {
    ...
    createRun: async (input: { connectionId: string; companyId: string; retryOfRunId?: string | null }): Promise<CloudUpstreamRun> => {
      ...
      if (!preview.schemaCompatible) {
        throw badRequest("Cloud stack schema is not compatible with this local Summon version");
      }
      ...
      const now = new Date();
      const initialEvents = [
        event(now.toISOString(), "connect", "completed", "Connected to the target Summon Cloud stack."),
      ...
      .set({
        ...
        report: {
          ...
          errorMessage: optionalString(report.errorMessage)
            ?? "The server restarted while this cloud upstream run was running, so Summon marked it failed instead of leaving it stuck.",
...
function targetFromDiscovery(discovery: Record<string, unknown>): CloudUpstreamTarget {
  ...
  return {
    ...
    origin,
    product: optionalString(discovery.product) ?? "Summon Cloud",
