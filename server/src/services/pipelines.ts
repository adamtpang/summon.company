...
export function pipelineService(db: Db, deps: { heartbeat?: IssueAssignmentWakeupDeps } = {}) {
  ...
  async function buildBreakdownMechanicsPrompt(
    ...
    return [
      ...
      "```",
      "",
      `Summon creates each ${input.config.pieceNoun} in "${targetPipeline.name}" at "${targetStage.name}", sets parentCaseId and requestKey, and copies inherited fields automatically.`,
      input.config.advanceTo ? `After the call succeeds, Summon moves this item to "${input.config.advanceTo}".` : null,
      "",
      "Target item fields:",
      "",
