...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Summon will import the routine trigger without those limits.`);
  }
    ...
    company: {
      ...
      attachmentMaxBytes:
        typeof paperclipCompany.attachmentMaxBytes === "number" && Number.isFinite(paperclipCompany.attachmentMaxBytes)
          ...
          : null,
      operatingMode: paperclipCompany.operatingMode === "always_on" ? "always_on" : "manual",
      budgetMonthlyCents:
        typeof paperclipCompany.budgetMonthlyCents === "number" && Number.isFinite(paperclipCompany.budgetMonthlyCents)
          ? Math.max(0, Math.floor(paperclipCompany.budgetMonthlyCents))
          : 0,
  ...
  async function importBundle(
    ...
    if (input.target.mode === "new_company") {
      ...
      const created = await companies.create({
        ...
        operatingMode: include.company ? (sourceManifest.company?.operatingMode ?? "manual") : "manual",
        budgetMonthlyCents: include.company ? (sourceManifest.company?.budgetMonthlyCents ?? 0) : 0,
      ...
      if (include.company && sourceManifest.company && mode === "board_full") {
        const updated = await companies.update(targetCompany.id, {
          ...
          operatingMode: sourceManifest.company.operatingMode,
          budgetMonthlyCents: sourceManifest.company.budgetMonthlyCents,
