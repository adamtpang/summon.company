...
function statusToneClasses(status: RemoteSecretImportCandidate["status"]) {
 ...
 case "conflict":
 return "text-(--status-task-todo) border-(--status-task-todo)/40 dark:text-(--status-task-todo)";
 case "ready":
 default:
 return "text-(--status-task-done) border-(--status-task-done)/40 dark:text-(--status-task-done)";
 }
}
...
function RowResultBadge({ status }: { status: RemoteSecretImportRowResult["status"] }) {
 ...
 <Badge
 variant="outline"
 className="gap-1 px-1.5 py-0 font-normal text-(--status-task-done) border-(--status-task-done)/40 dark:text-(--status-task-done)"
 >
...
function validateDraftRow(
 ...
 return "A Summon secret already uses this name.";
 }
 if (existingSecret.key.trim().toLowerCase() === lowerKey) {
 return "A Summon secret already uses this key.";
 }
 }
...
export function ImportFromVaultDialog({
 ...
 Bring AWS-managed secrets into Summon as external references.
 </DialogDescription>
 <Stepper step={step} />
 </div>
 <button
...
function SelectStep(props: SelectStepProps) {
 ...
 <Badge variant="outline" className="px-1 py-0 text-(length:--text-nano) text-(--status-task-todo) border-(--status-task-todo)/40">warning</Badge>
 )}
 {blocked && (
 ...
 <div className="mt-0.5 text-(length:--text-micro) text-(--status-task-todo) dark:text-(--status-task-todo)">
...
function ReviewStep({ drafts, reviewErrors, updateDraft, removeDraft, importing }: ReviewStepProps) {
 ...
 {blocked > 0 && (
 <span className="text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 error && "border-l-2 border-l-(--status-task-todo)/60 bg-(--status-task-todo)/5",
 )}
 ...
 <span className="text-muted-foreground">Summon name</span>
 <Input
 value={draft.name}
 onChange={(e) =>
 ...
 {error && (
 <div
 className="flex items-center gap-1.5 text-xs text-(--status-task-todo) dark:text-(--status-task-todo)"
 role="alert"
...
function ResultStep({ result, draftList }: ResultStepProps) {
 ...
 <span className="text-(--status-task-done) dark:text-(--status-task-done)">✓ {result.importedCount} created</span>
...
function FooterStatus({
 ...
 {blockedReviewCount > 0 && (
 <span className="ml-2 text-(--status-task-todo) dark:text-(--status-task-todo)">
