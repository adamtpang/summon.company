...
function EnvironmentCustomImageBrowserTerminal({
 ...
 <div className="bg-(--background) p-2 focus-within:ring-2 focus-within:ring-ring">
 <div
 ...
 className="h-(--sz-18rem) w-full overflow-hidden bg-(--background) outline-none sm:h-(--sz-22rem) [&_.xterm-cursor-bar]:!border-l-2 [&_.xterm-cursor-bar]:!border-l-(--status-task-in_progress) [&_.xterm-cursor-layer_.xterm-cursor]:!bg-(--status-task-in_progress) [&_.xterm-helper-textarea]:!opacity-0 [&_.xterm-screen]:focus:outline-none [&_.xterm-viewport]:!overflow-y-auto [&_.xterm]:h-full"
 />
 </div>
 {errorMessage ? (
...
function EnvironmentImageTemplatePanel({
 ...
 title={templateRef
 ? `Provider ${activeTemplate.templateKind} ref ${templateRef} (Summon template ${activeTemplate.id})`
 : activeTemplate.id}
 >
...
export function CompanyEnvironments({ mode = "list" }: CompanyEnvironmentsProps) {
 ...
 <div className="text-xs text-muted-foreground">Runs on this Summon host.</div>
 )}
 </div>
 <div className="flex flex-wrap items-center gap-2">
 ...
 className={
 probe.ok
 ? "mt-3 rounded border border-(--status-task-done)/30 bg-(--status-task-done)/5 px-2.5 py-2 text-xs text-(--status-task-done)"
 ...
 Configure a reusable execution target for your agents. Saved changes affect future runs; Summon may start fresh sessions or sandbox leases after environment config changes.
 </p>
 </div>
 ...
 <Field label="Remote workspace path" hint="Absolute path that Summon will verify during SSH connection tests.">
 <input
 className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
 type="text"
 ...
 <div className={draftEnvironmentProbeMutation.data.ok ? "text-xs text-(--status-task-done)" : "text-xs text-destructive"}>
