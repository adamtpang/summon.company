...
const MATERIALIZATION_NOTE =
 "Enabled skills are materialized into the stable Summon-managed prompt bundle on the agent's next run.";
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
 ...
 return "Summon cannot manage skills for custom ACP commands yet.";
 }
 if (agent.adapterType === "openclaw_gateway") {
 return "Summon cannot manage OpenClaw skills here. Visit your OpenClaw instance to manage this agent's skills.";
 }
 return "Summon cannot manage skills for this adapter yet. Manage them in the adapter directly.";
 ...
 <div className="overflow-hidden rounded-lg border border-(--status-task-todo)/60 dark:border-(--status-task-todo)/30">
 {staleDesiredKeys.map((key) => (
 <div
 key={key}
 className="flex items-center justify-between gap-3 border-b border-(--status-task-todo)/40 bg-(--status-task-todo)/60 px-3 py-2 text-xs text-(--status-task-todo) last:border-b-0 dark:border-(--status-task-todo)/20 dark:text-(--status-task-todo)"
 >
 ...
 className="inline-flex shrink-0 items-center gap-1 rounded-md border border-(--status-task-todo)/50 px-2 py-0.5 font-medium transition-colors hover:bg-(--status-task-todo)/60 "
 >
 <X className="h-3 w-3" />
 Remove
 </button>
