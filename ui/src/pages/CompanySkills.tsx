...
function SourceFilterMenu({
 ...
 variant="ghost"
 size="icon-sm"
 className={cn("relative shrink-0", activeFilterCount > 0 &&"text-(--status-task-in_progress) dark:text-(--status-task-in_progress)")}
 title={activeFilterCount > 0 ? `Filters: ${activeFilterCount}` :"Filter"}
 >
 <Filter className="h-3.5 w-3.5"/>
 {activeFilterCount > 0 ? (
 <span className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-(--status-task-in_progress) text-(length:--text-nano) font-bold text-white">
 {activeFilterCount}
 </span>
 ) : null}
 </Button>
...
function TrustChip({ level }: { level: CompanySkillTrustLevel }) {
 ...
 className:"border-(--status-task-in_progress)/30 bg-(--status-task-in_progress)/10 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)",
 },
 scripts_executables: {
 icon: AlertTriangle,
 label:"Includes scripts",
 tooltip:"Ships executable scripts. Review before installing.",
 className:"border-(--status-task-todo)/40 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)",
 },
 } as const;
...
"group flex h-full min-h-(--sz-11_5rem) flex-col rounded-lg border border-border bg-card p-4 text-left cursor-pointer transition-colors hover:border-foreground/20 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
 ...
 {card.installed ? (
 <Badge variant="outline"className="border-(--status-task-done)/30 bg-(--status-task-done)/10 text-(length:--text-nano) text-(--status-task-done) dark:text-(--status-task-done)">
 Installed
 </Badge>
 ) : null}
...
function CatalogDetailPane({
 ...
 cta = (
 <Button onClick={onUpdate} disabled={loadingPrimaryAction} className="border-(--status-task-todo)/40 bg-(--status-task-todo)/20 text-(--status-task-todo) dark:text-(--status-task-todo) hover:bg-(--status-task-todo)/30">
 ...
 <Tooltip>
 <TooltipTrigger asChild>
 <Badge variant="outline"className="border-(--status-task-todo)/40 bg-(--status-task-todo)/10 text-(length:--text-micro) text-(--status-task-todo) dark:text-(--status-task-todo)">
...
function InstallPreviewDialog({
 ...
 <span className="text-(--status-task-todo) dark:text-(--status-task-todo)">Review required</span>
 ) : (
 ...
 {conflict ? (
 <div className="rounded-md border border-(--status-task-todo)/40 bg-(--status-task-todo)/10 p-3 text-xs text-(--status-task-todo) dark:text-(--status-task-todo)">
...
"flex items-start gap-2 px-3 py-1.5 text-sm hover:bg-accent/30",
 ...
 {agent.paused ? (
 <Badge variant="outline"className="[&>svg]:size-2.5 border-(--status-task-todo)/30 bg-(--status-task-todo)/10 px-1.5 text-(length:--text-nano) uppercase tracking-wide text-(--status-task-todo)">
...
function SkillVersionDiffDialog({
 ...
 removed:"bg-(--status-task-blocked)/10 text-(--destructive) dark:text-(--destructive)",
 added:"bg-(--status-task-done)/10 text-(--status-task-done) dark:text-(--status-task-done)",
 };
 ...
 <label className="flex items-center gap-2">
 <Badge variant="outline"className="border-(--destructive)/30 bg-(--status-task-blocked)/10 uppercase tracking-wider text-(--destructive)">Old</Badge>
 <select
 value={leftVersionId ??""}
 ...
 <label className="flex items-center gap-2">
 <Badge variant="outline"className="border-(--status-task-done)/30 bg-(--status-task-done)/10 uppercase tracking-wider text-(--status-task-done)">New</Badge>
 <select
 value={right?.id ??""}
...
"Installs"counts agents that currently have this skill attached
 ...
 >
 <Star className={cn("h-3.5 w-3.5", detail.starredByCurrentActor &&"fill-current text-(--status-task-todo)")} />
 ...
 {meta?.paused ? (
 <Pause className="h-3 w-3 shrink-0 text-(--status-task-todo)"aria-label="Paused"/>
 ) : null}
 </Link>
 );
 })}
...
function SkillPane({
 ...
 <Tooltip>
 <TooltipTrigger asChild>
 <Badge variant="outline"className="border-(--status-task-in_review)/40 bg-(--status-task-in_review)/10 text-(length:--text-micro) text-(--status-task-in_review)">
