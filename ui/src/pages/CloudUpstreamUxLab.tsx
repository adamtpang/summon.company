...
function CloudUpstreamRender({ fixture }: { fixture: Fixture }) {
 ...
 Push {selectedCompanyName} into a Summon Cloud stack. Automations stay paused until activation.
 </p>
 </div>
 ...
 {notice ? (
 <div className="rounded-md border border-(--status-task-done)/30 bg-(--status-task-done)/5 px-3 py-2 text-sm text-(--status-task-done) dark:text-(--status-task-done)">
 {notice}
 </div>
 ) : null}
 {actionError ? (
 ...
 aria-label="Summon Cloud stack URL"
 autoFocus
 />
 <Button disabled>
...
function WarningsPanel({ warnings }: { warnings: CloudUpstreamWarning[] }) {
 ...
 <AlertTriangle className={warning.severity === "blocker" ? "h-4 w-4 text-destructive" : "h-4 w-4 text-(--status-task-todo)"} />
...
function buildFixture(state: FixtureStateKey): Fixture {
 ...
 case "settings-pane":
 return {
 selectedCompanyName: "Summon · PC521D",
