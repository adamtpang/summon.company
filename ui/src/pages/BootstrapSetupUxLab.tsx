...
function CliFallback({ hasActiveInvite }: { hasActiveInvite: boolean }) {
 ...
 {hasActiveInvite
 ? "A bootstrap invite is already active. Check your Summon startup logs for the first‑admin URL, or run this command on the host to rotate it:"
 : "Run this command on the host that runs Summon to print a one‑time first‑admin invite URL:"}
 </p>
...
function SignedOutPrivate() {
 return (
 <StateChrome>
 <h1 className="text-xl font-semibold">Finish setting up this Summon</h1>
 <p className="mt-2 text-sm text-muted-foreground">
 No admin has claimed this instance yet. Sign in or create your Summon account to become the first
...
function PublicInviteOnly() {
 return (
 <StateChrome>
 <h1 className="text-xl font-semibold">This Summon is waiting on its first admin</h1>
 <p className="mt-2 text-sm text-muted-foreground">
