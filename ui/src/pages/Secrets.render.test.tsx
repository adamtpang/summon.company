...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Summon config", async () => {
    ...
    expect(document.body.textContent).toContain("from Summon only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Summon"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
