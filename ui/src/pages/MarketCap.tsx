...
export function MarketCapPanel({ companyId }: { companyId: string | null }) {
 ...
 <div className="mt-4 flex items-start gap-2 rounded-md border border-(--status-task-todo)/30 bg-(--status-task-todo)/10 p-3">
 <Gauge className="mt-0.5 h-4 w-4 shrink-0 text-(--status-task-todo) dark:text-(--status-task-todo)" aria-hidden />
 ...
 lever.key === snapshot.bindingLever.key && "bg-(--status-task-todo)/5",
 )}
 >
 <td className="p-3">
 ...
 <span className="ml-2 rounded-full border border-(--status-task-todo)/40 px-2 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-(--status-task-todo) dark:text-(--status-task-todo)">
 binding
 </span>
 )}
 </div>
