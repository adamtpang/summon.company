...
export function PluginSettings() {
 ...
 <AlertTriangle className="h-3 w-3 text-(--status-task-todo)"/>
 Crashes
 </span>
 ...
 {check.passed ? (
 <CheckCircle className="h-4 w-4 shrink-0 text-(--status-task-done)"/>
 ) : (
...
function PluginLocalFolderRow({ pluginId, companyId, declaration, status }: PluginLocalFolderRowProps) {
 ...
 ?"border-(--status-task-done)/30 bg-(--status-task-done)/10 text-(--status-task-done) dark:border-(--status-task-done)/30 dark:text-(--status-task-done)"
...
function RequirementList({
 ...
 inspectionUnavailable
 ?"border-(--status-task-todo)/60 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:border-(--status-task-todo)/70 dark:text-(--status-task-todo)"
 : missing
 ...
 {inspectionUnavailable ? (
 <p className="text-xs text-(--status-task-todo) dark:text-(--status-task-todo)">Configured root was not inspected.</p>
 ) : missingItems.length > 0 ? (
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
 ...
 ?"text-(--status-task-done) bg-(--status-task-done)/10 border-(--status-task-done)/30 dark:text-(--status-task-done) dark:border-(--status-task-done)/30"
...
function JobStatusDot({ status }: { status: string }) {
 ...
 : status ==="running"
 ?"bg-(--status-task-in_progress) animate-pulse"
 : status ==="cancelled"
 ?"bg-(--accent)"
 :"bg-(--status-task-todo)"; // queued, pending
 return (
 <span
