...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Summon will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Summon handles this" mechanics bullets, composed from the
