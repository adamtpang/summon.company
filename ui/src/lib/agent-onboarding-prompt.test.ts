...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Summon API key");
    ...
    expect(prompt).toContain("Summon maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Summon host");
    expect(prompt).toContain("Hermes-originated Summon API calls");
    expect(prompt).toContain("Do not rotate or invent a Summon key manually");
