...
"h-4 w-4",
 ...
 !pending && starred && !error &&"fill-(--status-task-todo) text-(--status-task-todo)",
 !pending && error &&"text-(--destructive)",
 )}
 />
 <span>{label}</span>
 </Button>
...
"h-3.5 w-3.5",
 pending &&"motion-safe:animate-spin",
 !pending && error &&"text-(--destructive)",
 !pending && !error && starred &&"fill-(--status-task-todo) text-(--status-task-todo)",
