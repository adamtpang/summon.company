...
export function ProjectWorkspaceSummaryCard({
 ...
 hasRunningServices ? "bg-(--status-task-done)" : "bg-muted-foreground/40",
 )}
 />
 ...
 ? "text-(--status-task-done) hover:text-(--status-task-done) dark:text-(--status-task-done) dark:hover:text-(--status-task-done)"
 : "text-foreground",
 )}
 >
 {summary.primaryServiceUrl}
