...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
 ...
 return {
 icon: <Lock aria-hidden="true"className="h-6 w-6 text-(--status-task-todo)"/>,
 ...
 return {
 icon: <Ban aria-hidden="true"className="h-6 w-6 text-(--destructive)"/>,
 ...
 return {
 icon: <AlertTriangle aria-hidden="true"className="h-6 w-6 text-(--status-task-todo)"/>,
...
"truncate font-mono text-xs",
 ...
 <Check className="h-4 w-4 text-(--status-task-done)"/>
 ) : (
