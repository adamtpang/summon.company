...
export function IssueFiltersPopover({
 ...
 <Button variant={buttonVariant} size={iconOnly ? "icon" : "sm"} className={`text-xs ${iconOnly ? "relative h-8 w-8 shrink-0" : ""} ${activeFilterCount > 0 ? "text-(--status-task-in_progress) dark:text-(--status-task-in_progress)" : ""}`} title={iconOnly ? (activeFilterCount > 0 ? `Filters: ${activeFilterCount}` : "Filter") : undefined}>
 ...
 {iconOnly && activeFilterCount > 0 ? <span className="absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-(--status-task-in_progress) text-(length:--text-nano) font-bold text-white">{activeFilterCount}</span> : null}
