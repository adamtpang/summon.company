...
function SaveIndicator({ state }: { state: ProjectFieldSaveState }) {
 ...
 if (state ==="saved") {
 return (
 <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-(--status-task-done) dark:text-(--status-task-done)">
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
 ...
 <div className="text-(length:--text-micro) text-muted-foreground">Summon-managed folder.</div>
 )}
 </div>
 <div className="flex items-center gap-1">
 <Button
 ...
 Additional legacy workspace records exist on this project. Summon is using the primary workspace as the codebase view.
 </div>
 )}
...
"px-1.5 text-(length:--text-nano) uppercase tracking-wide",
 ...
 ?"bg-(--status-task-done)/15 text-(--status-task-done) dark:text-(--status-task-done)"
 : service.status ==="failed"
 ?"bg-(--status-task-blocked)/15 text-(--destructive) dark:text-(--destructive)"
