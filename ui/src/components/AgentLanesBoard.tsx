...
export function AgentLanesBoard({ issues, agents, liveIssueIds, issueLinkState }: AgentLanesBoardProps) {
 ...
 className={path.active ?"stroke-(--status-task-blocked)/70":"stroke-muted-foreground/40"}
 ...
 <span className="text-sm font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">Unassigned</span>
 ) : (
...
"hover:border-foreground/20 hover:shadow-sm",
 ...
 <span className="relative inline-flex h-2 w-2 rounded-full bg-(--status-task-in_progress)"/>
 </span>
 ) : null}
