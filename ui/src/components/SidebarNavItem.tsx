...
"flex items-center gap-2.5 mx-2 rounded-lg px-2 py-1.5 pointer-coarse:py-1 text-(length:--text-compact) font-medium transition-colors",
 ...
 {alert && (
 <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-(--status-task-blocked) shadow-(--shadow-extract-12)"aria-hidden="true"/>
 )}
 ...
 <span className="animate-pulse absolute inline-flex h-full w-full rounded-full bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress) opacity-75"/>
 <span className="relative inline-flex h-2 w-2 rounded-full bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress) shadow-(--shadow-extract-12)"/>
 </span>
 )}
...
"absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full shadow-(--shadow-extract-12)",
 badgeTone ==="danger"?"bg-(--status-task-blocked)":"bg-primary",
 )}
 aria-hidden="true"
 />
 )}
...
"ml-auto px-1.5 text-(length:--text-nano) leading-none",
 textBadgeTone ==="amber"
 ?"bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)"
 ...
 <span className="animate-pulse absolute inline-flex h-full w-full rounded-full bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress) opacity-75"/>
 <span className="relative inline-flex rounded-full h-2 w-2 bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress)"/>
 </span>
 <span className="text-(length:--text-micro) font-medium text-(--status-task-in_progress) dark:text-(--status-task-in_progress)">{liveCount} live</span>
 </>
 )}
 </span>
 )}
...
"ml-auto px-1.5 leading-none",
 badgeTone ==="danger"
 ?"bg-(--status-task-blocked)/90 text-(--destructive)"
