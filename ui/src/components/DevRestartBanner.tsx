...
export function DevRestartBanner({ devServer }: { devServer?: DevServerHealthStatus }) {
 ...
 ? `Restart Summon now? This may interrupt ${activeRunLabel}.`
 : "Restart Summon now?";
 ...
 return (
 <div className="border-b border-(--status-task-todo)/60 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:border-(--status-task-todo)/25 dark:bg-(--status-task-todo)/10 dark:text-(--status-task-todo)">
 ...
 <Badge variant="ghost" className="bg-(--status-task-todo)/10 text-(length:--text-nano) tracking-(--tracking-eyebrow) dark:bg-(--status-task-todo)/10">
 Auto-Restart On
 </Badge>
 ) : null}
 </div>
 ...
 <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-(--muted-foreground) dark:text-(--muted-foreground)">
 {sample.length > 0 ? (
 <span>
 Changed: {sample.join(", ")}
 ...
 <div className="inline-flex items-center gap-2 rounded-full bg-(--status-task-todo)/10 px-3 py-1.5 dark:bg-(--status-task-todo)/10">
 ...
 <button
 type="button"
 className="inline-flex items-center gap-2 rounded-md bg-(--status-task-todo)/20 px-3 py-1.5 text-xs font-semibold text-(--status-task-todo) transition-colors hover:bg-(--status-task-todo)/25 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-(--status-task-todo)/10 dark:text-(--status-task-todo) dark:hover:bg-(--status-task-todo)/10"
 onClick={() => {
