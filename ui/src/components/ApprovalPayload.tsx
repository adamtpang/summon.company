...
function BoardApprovalPayloadContent({ payload }: { payload: Record<string, unknown> }) {
 ...
 {recommendedAction && (
 <div className="rounded-lg border border-(--status-task-todo)/20 bg-(--status-task-todo)/10 px-3.5 py-3">
 <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-(--status-task-todo) dark:text-(--status-task-todo)">
 Recommended action
 </p>
