...
export function ProductivityReviewBadge({
 ...
 to={reviewPath}
 className={cn(
 "inline-flex items-center gap-1 rounded-full border border-(--status-task-todo)/40 bg-(--status-task-todo)/10 px-2 py-0.5 text-(length:--text-nano) font-medium text-(--status-task-todo) dark:text-(--status-task-todo) shrink-0 hover:bg-(--status-task-todo)/20 transition-colors",
 className,
 )}
