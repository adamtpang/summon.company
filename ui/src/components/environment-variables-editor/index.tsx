...
export const EnvironmentVariablesEditor = forwardRef<EnvironmentVariablesEditorHandle, EnvironmentVariablesEditorProps>(function EnvironmentVariablesEditor({
 ...
 {attentionCount > 1 ? (
 <p className="inline-flex items-center gap-1.5 text-(length:--text-micro) font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 <div
 role="status"
 className="mt-3 flex w-full flex-col gap-3 rounded-md border border-(--status-task-todo)/30 bg-(--status-task-todo)/10 px-3 py-3 text-(--status-task-todo) shadow-sm dark:bg-(--status-task-todo)/15 dark:text-(--status-task-todo) @[34rem]/env:flex-row @[34rem]/env:items-center @[34rem]/env:justify-between"
 >
 <div className="flex min-w-0 flex-col gap-0.5">
 <div className="flex items-center gap-2 text-sm font-medium">
 <span className="size-2 rounded-full bg-(--status-task-todo) shadow-(--shadow-extract-13)" />
 <span>Unsaved changes</span>
 </div>
 {changeSummaryText ? (
 <p className="min-w-0 truncate pl-4 text-xs text-(--muted-foreground) dark:text-(--muted-foreground)" title={changeSummaryText}>
 {changeSummaryText}
 </p>
 ) : null}
 </div>
 ...
 type="button"
 onClick={revertDraft}
 className="inline-flex h-9 items-center gap-1.5 rounded-md border border-(--status-task-todo)/30 bg-background px-3 text-sm font-medium text-foreground transition-colors hover:bg-(--status-task-todo)/10 dark:bg-background/80"
 >
