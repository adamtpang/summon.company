...
export function EnvironmentVariableRow({
 ...
 isDirty && "bg-(--status-task-todo)/[0.06] ring-1 ring-(--status-task-todo)/20",
 )}
 >
 ...
 className={cn(
 nameInputClass,
 dirtyFields.name && "border-(--status-task-todo)/70 bg-(--status-task-todo)/10 focus-visible:ring-(--status-task-todo)/40",
 showNameIssue && nameIssue?.level === "error" && "border-destructive focus-visible:ring-destructive/40",
 showNameIssue && nameIssue?.level === "warn" && "border-(--status-task-todo)/30 focus-visible:ring-(--status-task-todo)/40",
 )}
 placeholder="KEY"
 value={row.name}
 spellCheck={false}
 ...
 dirtyFields.value && "border-(--status-task-todo)/70 bg-(--status-task-todo)/10 focus-within:ring-(--status-task-todo)/40",
 disabled && "opacity-60",
 )}
 >
 ...
 disabled={disabled}
 className="flex items-center gap-1 px-2 text-(length:--text-micro) text-(--status-task-todo) hover:bg-(--status-task-todo)/10 dark:text-(--status-task-todo)"
 ...
 className="flex items-center px-1.5 text-(--muted-foreground) hover:bg-(--status-task-todo)/10 hover:text-(--status-task-todo) dark:text-(--muted-foreground) dark:hover:text-(--status-task-todo)"
 >
 <X className="size-3" />
 </button>
 </div>
 ...
 versionPinned
 ? "bg-(--status-task-todo)/15 text-(--status-task-todo) dark:text-(--status-task-todo)"
 ...
 row.version === v && "font-medium text-(--status-task-todo) dark:text-(--status-task-todo)",
 )}
 >
 v{v}
 </button>
 ...
 health.level === "error" ? "text-destructive" : "text-(--status-task-todo) dark:text-(--status-task-todo)",
 )}
 >
 {health.message}
 </p>
 ...
 nameIssue.level === "error" ? "text-destructive" : "text-(--status-task-todo) dark:text-(--status-task-todo)",
 )}
 >
 {nameIssue.message}
 </p>
