...
export function SecretPicker({
 ...
 boundSecret && boundSecret.status !== "active" && "border-(--status-task-todo)/60",
 triggerClassName,
 )}
 renderValue={(option) => {
 if (!option) {
 ...
 <KeyRound className={cn("size-3.5 shrink-0", nonActive ? "text-(--status-task-todo)" : "text-muted-foreground")} />
 <span className="min-w-0 flex-1 truncate">{option.label}</span>
 {nonActive ? <span className="text-(--status-task-todo)">({option.status})</span> : null}
 </span>
 );
 }}
