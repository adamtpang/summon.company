...
function statusTone(status: BudgetPolicySummary["status"]) {
 if (status ==="hard_stop") return"text-(--destructive) dark:text-(--destructive) border-(--destructive)/30 bg-(--status-task-blocked)/10";
 if (status ==="warning") return"text-(--status-task-todo) dark:text-(--status-task-todo) border-(--status-task-todo)/30 bg-(--status-task-todo)/10";
 return"text-(--status-task-done) dark:text-(--status-task-done) border-(--status-task-done)/30 bg-(--status-task-done)/10";
}
...
"h-full rounded-full transition-(--tp-width-background-color) duration-200",
 ...
 <div className="flex items-start gap-2 rounded-xl border border-(--destructive)/30 bg-(--status-task-blocked)/10 px-3 py-2 text-sm text-(--destructive) dark:text-(--destructive)">
