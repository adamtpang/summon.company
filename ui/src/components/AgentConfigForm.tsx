...
"promptTemplate",
 ...
 <div className="rounded-md border border-(--status-task-todo)/25 bg-(--status-task-todo)/10 px-3 py-2 text-xs text-(--status-task-todo) dark:text-(--status-task-todo)">
...
"adapterConfig",
 ...
 <p className="text-xs text-(--status-task-todo)">
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
 ...
 const statusClass =
 result.status ==="pass"
 ?"text-(--status-task-done) dark:text-(--status-task-done) border-(--status-task-done)/30 dark:border-(--status-task-done)/40 bg-(--status-task-done)/10 dark:bg-(--status-task-done)/10"
 : result.status ==="warn"
 ?"text-(--status-task-todo) dark:text-(--status-task-todo) border-(--status-task-todo)/30 dark:border-(--status-task-todo)/40 bg-(--status-task-todo)/10 dark:bg-(--status-task-todo)/10"
 :"text-(--destructive) dark:text-(--destructive) border-(--destructive)/20 dark:border-(--destructive)/40 bg-(--destructive)/10 dark:bg-(--status-task-blocked)/10";
...
function ExperimentalBadge() {
 return (
 <span className="shrink-0 rounded border border-(--status-task-todo)/30 bg-(--status-task-todo)/10 px-1.5 py-0.5 text-(length:--text-nano) font-medium leading-none text-(--status-task-todo) dark:text-(--status-task-todo)">
 Experimental
 </span>
 );
}
...
"flex items-center w-full px-2 py-1.5 text-sm rounded bg-accent/50",
 ...
 <Badge variant="outline"className="ml-auto text-(length:--text-nano) px-1.5 bg-(--status-task-done)/15 text-(--status-task-done) border-(--status-task-done)/20">
 current
 </Badge>
 </button>
 )}
...
"flex items-center w-full px-2 py-1.5 text-sm rounded hover:bg-accent/50",
 ...
 <Badge variant="outline"className="ml-auto text-(length:--text-nano) px-1.5 bg-(--status-task-in_progress)/15 text-(--status-task-in_progress) border-(--status-task-in_progress)/20">
 detected
 </Badge>
 </button>
 )}
...
function CheapModelSection({
 ...
 {enabled && !model && !adapterDefaultModel ? (
 <p className="text-(length:--text-micro) text-(--status-task-todo)">
