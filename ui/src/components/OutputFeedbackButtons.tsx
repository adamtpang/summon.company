...
export function OutputFeedbackButtons({
 ...
 className={cn(visibleVote === "up" && "border-(--status-task-done)/50 bg-(--status-task-done)/10 text-(--status-task-done)")}
 ...
 variant="outline"
 disabled={disabled || isSaving}
 className={cn(visibleVote === "down" && "border-(--status-task-todo)/50 bg-(--status-task-todo)/10 text-(--status-task-todo)")}
