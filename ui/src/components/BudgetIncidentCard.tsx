...
export function BudgetIncidentCard({
 ...
 return (
 <Card className="overflow-hidden border-(--destructive)/20 bg-(image:--gradient-extract-4)">
 <CardHeader className="px-5 pt-5 pb-3">
 <div className="flex items-start justify-between gap-3">
 <div>
 <div className="flex flex-wrap items-center gap-2">
 <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-(--destructive)/90 dark:text-(--destructive)/80">
 ...
 <CardTitle className="mt-1 text-base text-(--destructive) dark:text-(--destructive)">{incident.scopeName}</CardTitle>
 <CardDescription className="mt-1 text-(--destructive)/75 dark:text-(--destructive)/70">
 Spending reached {formatCents(incident.amountObserved)} against a limit of {formatCents(incident.amountLimit)}.
 </CardDescription>
 </div>
 <div className="rounded-full border border-(--destructive)/25 bg-(--status-task-blocked)/10 p-2 text-(--destructive) dark:text-(--destructive)">
 <AlertOctagon className="h-4 w-4"/>
 </div>
 </div>
 </CardHeader>
 <CardContent className="space-y-4 px-5 pb-5 pt-0">
 <div className="flex items-start gap-2 rounded-xl border border-(--destructive)/25 bg-(--status-task-blocked)/10 px-3 py-2 text-sm text-(--destructive)/90 dark:text-(--destructive)/90">
 ...
 <p className="mt-2 text-xs text-(--destructive) dark:text-(--destructive)/80">
