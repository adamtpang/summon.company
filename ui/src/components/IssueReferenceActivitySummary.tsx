...
export function IssueReferenceActivitySummary({ event }: { event: Pick<ActivityEvent, "details"> }) {
 ...
 icon={<Plus className="h-3 w-3 text-(--status-task-done) dark:text-(--status-task-done)" aria-hidden="true" />}
 items={added}
 />
 <Section
 label="Removed references"
 icon={<Minus className="h-3 w-3 text-(--destructive)" aria-hidden="true" />}
 items={removed}
 strikethrough
 />
 </div>
