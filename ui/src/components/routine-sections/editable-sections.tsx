...
export function OverviewSection({
 ...
 <div className="rounded-lg border border-(--status-task-todo)/30 bg-(--status-task-todo)/5 p-4 text-sm text-(--status-task-todo) dark:text-(--status-task-todo)">
...
export function SecretsSection() {
 ...
 {secretMessage ? (
 <div className="space-y-3 rounded-lg border border-(--status-task-in_progress)/30 bg-(--status-task-in_progress)/5 p-4 text-sm">
 <div>
 <p className="font-medium">{secretMessage.title}</p>
 <p className="text-xs text-muted-foreground">
 Save this now. Summon will not show the secret value again.
 </p>
 </div>
 <div className="space-y-3">
...
const dispositionToneClass: Record<string, string> = {
 queued: "text-(--status-task-done) dark:text-(--status-task-done)",
 coalesced: "text-(--status-task-todo) dark:text-(--status-task-todo)",
