...
function CliFallback({ hasActiveInvite = false }: { hasActiveInvite?: boolean }) {
 ...
 {hasActiveInvite
 ? `A bootstrap invite is already active. Check your ${APP_NAME} startup logs for the first-admin URL, or run this command on the host to rotate it:`
 : `Run this command on the host that runs ${APP_NAME} to print a one-time first-admin invite URL:`}
 </p>
...
export function BootstrapPendingPage({
 ...
 return (
 <StateChrome>
 <h1 className="text-xl font-semibold">This {APP_NAME} instance is waiting on its first admin</h1>
 <p className="mt-2 text-sm text-muted-foreground">
 ...
 <div className="mt-0.5 flex size-9 flex-shrink-0 items-center justify-center rounded-full bg-(--status-task-done)/15 text-(--status-task-done) dark:text-(--status-task-done)">
 <ShieldCheck className="size-5" aria-hidden />
 </div>
 <div>
 <h1 className="text-xl font-semibold">You're the instance admin</h1>
 <p className="mt-2 text-sm text-muted-foreground">
 Setup is complete. Taking you to onboarding to hire your first AI employee...
 </p>
 </div>
 </div>
 ...
 return (
 <StateChrome>
 <h1 className="text-xl font-semibold">Finish setting up this {APP_NAME} instance</h1>
 <p className="mt-2 text-sm text-muted-foreground">
 No admin has claimed this instance yet. Sign in or create your {APP_NAME} account to become the first
