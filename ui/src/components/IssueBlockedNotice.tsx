...
function SuccessfulRunRetryNowControl({
 ...
 return (
 <div className="mt-2 rounded-md border border-(--status-task-todo)/70 bg-background/80 p-2 dark:border-(--status-task-todo)/40 dark:bg-background/40">
 <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
 <div className="min-w-0 text-xs leading-5 text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 variant="outline"
 size="sm"
 className="shrink-0 border-(--status-task-todo)/80 bg-background/80 text-(--status-task-todo) shadow-none hover:bg-(--status-task-todo)/10 dark:border-(--status-task-todo)/50 dark:bg-background/40 dark:text-(--status-task-todo) dark:hover:bg-(--status-task-todo)/15"
 ...
 className="mt-2 border-(--status-task-todo)/70 bg-(--status-task-todo)/70 text-(--status-task-todo) dark:border-(--status-task-todo)/40 dark:bg-(--status-task-todo)/15 dark:text-(--status-task-todo)"
 onRetry={() => {
 retryNow.reset();
 retryNow.mutate();
 }}
...
function WaitingChipLink({
 ...
 className="inline-flex max-w-full items-center gap-1 rounded-md border border-(--status-task-in_progress)/70 bg-background/80 px-2 py-1 font-mono text-xs text-(--status-task-in_progress) transition-colors hover:border-(--status-task-in_progress)/30 hover:bg-(--status-task-in_progress)/10 hover:underline dark:border-(--status-task-in_progress)/40 dark:bg-background/40 dark:text-(--status-task-in_progress) dark:hover:bg-(--status-task-in_progress)/15"
 >
 <StatusGlyph
 ...
 <span className="max-w-(--sz-18rem) truncate font-sans text-(length:--text-micro) text-(--status-task-in_progress) dark:text-(--status-task-in_progress)">
 {blocker.title}
 </span>
 {running ? (
 <span className="ml-0.5 rounded-full bg-(--status-task-in_progress)/15 px-1.5 py-0.5 text-(length:--text-nano) font-medium uppercase tracking-wide text-(--status-task-in_progress) dark:bg-(--status-task-in_progress)/20 dark:text-(--status-task-in_progress)">
 running
 </span>
 ) : null}
 </IssueLinkQuicklook>
...
function WaitingStepGlyph({ status }: { status: WaitingStepStatus }) {
 if (status === "done") {
 return <CheckCircle2 className="h-3.5 w-3.5 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)" aria-hidden />;
 }
 if (status === "running") {
 return (
 ...
 return <Circle className="h-3.5 w-3.5 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)/50" aria-hidden />;
}
...
function WaitingOnLiveWorkNotice({
 ...
 className="mb-3 rounded-md border border-(--status-task-in_progress)/70 bg-(--status-task-in_progress)/90 px-3 py-2.5 text-sm text-(--status-task-in_progress) shadow-sm dark:border-(--status-task-in_progress)/40 dark:bg-(--status-task-in_progress)/10 dark:text-(--status-task-in_progress)"
 >
 ...
 <div className="text-xs font-medium text-(--status-task-in_progress) dark:text-(--status-task-in_progress)">
 ...
 aria-valuemax={total}
 className="flex h-2 w-full overflow-hidden rounded-full bg-(--status-task-in_progress)/10 dark:bg-(--status-task-in_progress)/20"
 >
 {steps.map(({ blocker, status }) => (
 <span
 key={blocker.id}
 className={cn(
 "h-full border-r border-(--status-task-in_progress)/80 last:border-r-0 dark:border-(--status-task-in_progress)/40",
 status === "done"
 ? "bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress)"
 : status === "running"
 ? "animate-pulse bg-(--status-task-in_progress)"
 : "bg-(--status-task-in_progress)/10 dark:bg-(--status-task-in_progress)/30",
 )}
 ...
 <span
 className="w-px flex-1 bg-(--status-task-in_progress)/20 dark:bg-(--status-task-in_progress)/30"
 aria-hidden
 />
 </div>
 ...
 <div className="flex w-3.5 flex-col items-center">
 <span
 className="mt-1.5 h-3 w-3 rounded-full border border-dashed border-(--status-task-in_progress)/60 dark:border-(--status-task-in_progress)/50"
 aria-hidden
 />
 </div>
 <div className="min-w-0 pb-0.5">
 <span className="inline-block rounded-md border border-dashed border-(--status-task-in_progress)/70 px-2 py-1 text-xs text-(--status-task-in_progress) dark:border-(--status-task-in_progress)/40 dark:text-(--status-task-in_progress)">
 ...
 >
 <span className="inline-flex items-center gap-1 text-xs font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 */
 ...
 issuePathId={issuePathId}
 to={createIssueDetailPath(issuePathId)}
 className="inline-flex max-w-full items-center gap-1 rounded-md border border-(--status-task-todo)/70 bg-background/80 px-2 py-1 font-mono text-xs text-(--status-task-todo) transition-colors hover:border-(--status-task-todo)/30 hover:bg-(--status-task-todo)/10 hover:underline dark:border-(--status-task-todo)/40 dark:bg-background/40 dark:text-(--status-task-todo) dark:hover:bg-(--status-task-todo)/15"
 >
 <span>{blocker.identifier ?? blocker.id.slice(0, 8)}</span>
 <span className="max-w-(--sz-18rem) truncate font-sans text-(length:--text-micro) text-(--status-task-todo) dark:text-(--status-task-todo)">
 {blocker.title}
 </span>
 ...
 className="mb-3 rounded-md border border-(--status-task-todo)/70 bg-(--status-task-todo)/90 px-3 py-2.5 text-sm text-(--status-task-todo) shadow-sm dark:border-(--status-task-todo)/40 dark:bg-(--status-task-todo)/10 dark:text-(--status-task-todo)"
 >
 <div className="flex items-start gap-2">
 <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-(--status-task-todo) dark:text-(--status-task-todo)" />
 ...
 <code className="rounded bg-(--status-task-todo)/10 px-1 py-0.5 text-xs dark:bg-(--status-task-todo)/15">
 in_progress
 </code>{" "}
 with no clear owner for the next action.
 </p>
 <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 className="rounded-md border border-(--status-task-todo)/70 bg-background/80 px-2 py-1 font-mono text-(--status-task-todo) hover:border-(--status-task-todo)/30 hover:bg-(--status-task-todo)/10 hover:underline dark:border-(--status-task-todo)/40 dark:bg-background/40 dark:text-(--status-task-todo) dark:hover:bg-(--status-task-todo)/15"
 >
 run {successfulRunHandoff.sourceRunId.slice(0, 8)}
 </Link>
 ) : successfulRunHandoff.sourceRunId ? (
 <span className="rounded-md border border-(--status-task-todo)/70 bg-background/80 px-2 py-1 font-mono text-(--status-task-todo) dark:border-(--status-task-todo)/40 dark:bg-background/40 dark:text-(--status-task-todo)">
 run {successfulRunHandoff.sourceRunId.slice(0, 8)}
 </span>
 ) : null}
 <span className="rounded-md border border-(--status-task-todo)/70 bg-background/80 px-2 py-1 text-(--status-task-todo) dark:border-(--status-task-todo)/40 dark:bg-background/40 dark:text-(--status-task-todo)">
 Corrective wake queued for {agentName ?? "the responsible"}
 </span>
 </div>
 {successfulRunHandoff.detectedProgressSummary ? (
 <p className="text-xs leading-5 text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 <div className="border-t border-(--status-task-todo)/60 pt-1.5 dark:border-(--status-task-todo)/30" />
 ) : null}
 ...
 <span className="text-xs font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">
 Stalled in review
 </span>
