...
function statusBadge(status: string) {
 ...
 case"active":
 case"draft":
 return { label:"In Progress", className:"bg-(--status-task-in_progress)/10 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)"};
 case"ready_for_review":
 return { label:"For Review", className:"bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)"};
 case"approved":
 case"merged":
 return { label:"Completed", className:"bg-(--status-task-done)/10 text-(--status-task-done) dark:text-(--status-task-done)"};
 case"changes_requested":
 return { label:"Changes Requested", className:"bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)"};
 case"failed":
 return { label:"Failed", className:"bg-(--status-task-blocked)/10 text-(--destructive)"};
 default:
...
"w-full text-left px-4 py-3 hover:bg-accent/30 transition-colors",
 ...
 {showGenerating ? (
 <Badge variant="ghost"className="[&>svg]:size-2.5 text-(length:--text-nano) px-1.5 bg-(--status-task-in_progress)/10 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)">
...
function DocumentViewer({
 ...
 <Button size="lg"className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-(--status-task-done) hover:bg-(--status-task-done)/80 text-white border-0"onClick={onApprove}>
 Approve
 </Button>
 <Button size="lg"className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-(--status-task-blocked) hover:bg-(--status-task-blocked)/80 text-white border-0"onClick={() => {
 onReject?.();
 onBack();
 }}>
 Reject
 ...
 {isApproved && (
 <div className="border-t border-(--status-task-done)/30 bg-(--status-task-done)/5 px-4 py-3 shrink-0">
 <div className="flex items-center gap-2">
 <CheckCircle2 className="h-4 w-4 text-(--status-task-done)"/>
 <p className="text-(length:--text-compact) font-medium text-(--status-task-done) dark:text-(--status-task-done)">
 ...
 {isRejected && (
 <div className="border-t border-(--status-task-todo)/30 bg-(--status-task-todo)/5 px-4 py-3 shrink-0">
 <div className="flex items-center gap-2">
 <XCircle className="h-4 w-4 text-(--status-task-todo)"/>
 <p className="text-(length:--text-compact) font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">
