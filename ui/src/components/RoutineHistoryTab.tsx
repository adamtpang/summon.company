...
export function RoutineHistoryTab({
 ...
 body: error instanceof Error ? error.message :"Summon could not restore the revision.",
 tone:"error",
 });
 },
 });
...
function HistoricalPreviewBanner({
 ...
 return (
 <div className="rounded-md border border-(--status-task-todo)/30 bg-(--status-task-todo)/5 px-4 py-3">
...
function ConflictBanner({
 ...
 <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
 <div className="space-y-1">
 <p className="text-sm font-medium text-(--status-task-todo) dark:text-(--status-task-todo)">Unsaved routine edits</p>
 <p className="text-xs text-muted-foreground">
 ...
 <span className="h-1 w-1 rounded-full bg-(--status-task-todo)"/>
 {field.label}
 </li>
 ))}
 </ul>
...
function RevisionList({
 ...
 ?"border-(--status-task-done)/40 bg-(--status-task-done)/10"
 : isSelected && isHistorical
 ?"border-(--status-task-todo)/40 bg-(--status-task-todo)/10"
 : isSelected
 ?"border-border bg-accent/40"
 : blockedByEdits
 ?"border-(--status-task-todo)/30 bg-(--status-task-todo)/5 opacity-70 cursor-not-allowed"
 ...
 <Badge variant="outline"className="border-(--status-task-todo)/40 bg-(--status-task-todo)/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-(--status-task-todo) dark:text-(--status-task-todo)">
 Restored
 </Badge>
 )}
 </div>
...
function RevisionPreview({
 ...
 highlighted ?"border-(--status-task-done)/40 bg-(--status-task-done)/10":"border-border"
 }`;
 ...
 {row.differs && (
 <Badge variant="outline"className="ml-2 border-(--status-task-todo)/40 bg-(--status-task-todo)/10 px-1.5 text-(length:--text-nano) uppercase tracking-(--tracking-eyebrow) text-(--status-task-todo) dark:text-(--status-task-todo)">
 ...
 <span
 className={`ml-auto text-xs ${trigger.enabled ?"text-(--status-task-done)":"text-muted-foreground"}`}
 >
 ...
 Summon mints fresh secret material at restore time.
 </p>
 </div>
...
function RestoreConfirmDialog({
 ...
 <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-(--status-task-done)"/>
 ...
 <li key={label} className="flex items-start gap-2 text-(--status-task-todo) dark:text-(--status-task-todo)">
 <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-(--status-task-todo)"/>
...
function RoutineRevisionDiffModal({
 ...
 <td className="px-3 py-2 align-top text-xs text-(--destructive) dark:text-(--destructive)">
 {change.oldValue ??"·"}
 </td>
 <td className="px-3 py-2 align-top text-xs text-(--status-task-done) dark:text-(--status-task-done)">
 {change.newValue ??"·"}
 </td>
 </tr>
 ))}
...
function RevisionPicker({
 ...
 ?"border-(--destructive)/30 bg-(--status-task-blocked)/10 text-(--destructive) dark:text-(--destructive)"
 :"border-(--status-task-done)/30 bg-(--status-task-done)/10 text-(--status-task-done) dark:text-(--status-task-done)";
 return (
