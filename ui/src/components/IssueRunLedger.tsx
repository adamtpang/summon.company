...
const LIVENESS_COPY: Record<RunLivenessState, LivenessCopy> = {
 completed: {
 label:"Completed",
 tone:"border-(--status-task-done)/30 bg-(--status-task-done)/10 text-(--status-task-done) dark:text-(--status-task-done)",
 description:"Task reached a terminal state.",
 },
 advanced: {
 label:"Advanced",
 tone:"border-(--status-task-in_progress)/30 bg-(--status-task-in_progress)/10 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)",
 description:"Run produced concrete evidence of progress.",
 },
 plan_only: {
 label:"Plan only",
 tone:"border-(--status-task-todo)/30 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)",
 ...
 failed: {
 label:"Failed",
 tone:"border-(--destructive)/30 bg-(--status-task-blocked)/10 text-(--destructive) dark:text-(--destructive)",
...
const RETRY_PENDING_LIVENESS_COPY: LivenessCopy = {
 label:"Retry pending",
 tone:"border-(--status-task-in_progress)/30 bg-(--status-task-in_progress)/10 text-(--status-task-in_progress) dark:text-(--status-task-in_progress)",
 description:"Summon queued an automatic retry that has not started yet.",
};
...
function watchdogDecisionErrorMessage(error: unknown) {
 ...
 ? error.message
 :"Summon could not record the watchdog decision.";
}
...
"rounded-md border px-3 py-2 text-xs",
 ...
 :"border-(--status-task-todo)/30 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)",
 )}
 >
 ...
 {watchdogDecisionError ? (
 <p className="mt-2 rounded-md border border-(--destructive)/30 bg-(--status-task-blocked)/10 px-2 py-1 text-(length:--text-micro) text-(--destructive) dark:text-(--destructive)">
 {watchdogDecisionError}
 </p>
 ) : null}
 </div>
 ...
 {run.isLive ? (
 <span className="inline-flex items-center gap-1 rounded-md border border-(--status-task-in_progress)/30 bg-(--status-task-in_progress)/10 px-1.5 py-0.5 text-(length:--text-micro) text-(--status-task-in_progress) dark:text-(--status-task-in_progress)">
...
"rounded-md border px-1.5 py-0.5 text-(length:--text-micro) font-medium",
 ...
 {exhausted ? (
 <span className="rounded-md border border-(--destructive)/30 bg-(--status-task-blocked)/10 px-1.5 py-0.5 text-(length:--text-micro) font-medium text-(--destructive) dark:text-(--destructive)">
 Exhausted
 </span>
 ) : null}
 {continuation ? (
...
"rounded-md border px-1.5 py-0.5 text-(length:--text-micro) font-medium",
 ...
 return (
 <p className="min-w-0 break-words text-(length:--text-micro) leading-5 text-(--status-task-todo) dark:text-(--status-task-todo)">
