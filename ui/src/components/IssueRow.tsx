...
export function IssueRow({
 ...
 <span
 className={cn(
 "inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-(--status-task-todo)/40 bg-(--status-task-todo)/10 text-(--status-task-todo) dark:text-(--status-task-todo)",
 ...
 className="[&>svg]:size-2.5 ml-1.5 gap-0.5 border-(--status-task-todo)/60 bg-(--status-task-todo)/15 text-(length:--text-nano) text-(--status-task-todo) dark:text-(--status-task-todo)"
 ...
 selected ? "hover:bg-muted/80" : "hover:bg-(--status-task-in_progress)/20",
 )}
 ...
 selected ? "bg-muted-foreground/70" : "bg-(--status-task-in_progress) dark:bg-(--status-task-in_progress)",
